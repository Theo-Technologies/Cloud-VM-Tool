...
Start-Process -FilePath "C:\TheoTemp\Devcon\devcon.exe" -ArgumentList $arguement
}
Else {"Device is enabled"
Start-Process -FilePath "C:\TheoTemp\Devcon\devcon.exe" -ArgumentList '/m /r'}
}
...
$ShortCut.Arguments='-WindowStyle hidden -ExecutionPolicy Bypass -File "C:\TheoTemp\Drivers\NVSMI.ps1"'
$ShortCut.WorkingDirectory = "C:\TheoTemp\Drivers";
...
(New-Object System.Net.WebClient).DownloadFile($($($URL.GoogleGRID).links | Where-Object href -like *server2016_64bit_international.exe*).href, "C:\TheoTemp\Drivers\GoogleGRID.exe")
}
Else {
...
$system = @{Valid_NVIDIA_Driver = ValidDriver; OS_Version = osVersion; OS_Reboot_Required = RequiresReboot; Date = get-date; Path = "C:\TheoTemp\Drivers"}
